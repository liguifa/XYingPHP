...
class DataBase
{
	...
	****************************/
	...
	{
		self::$hostname=config::$dbConnect['host'];     
		self::$username=config::$dbConnect['user'];
		self::$password=config::$dbConnect['pwd'];
		self::$databasename=config::$dbConnect['database'];
		$this->SqlConnect();
	}
	...
	****************************/
	private  function sqlConnect()
	{
		...
		{
			self::$dbInstance = new PDO("mssql:dbname=".self::$databasename.";host=".self::$hostname,self::$username,self::$password);
			if(self::$dbInstance)echo "ok";
		}
